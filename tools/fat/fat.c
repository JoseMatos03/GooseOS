...
} __attribute__((packed)) BootSector; // remove padding from structs
...
bool readRootDirectory(FILE *disk) {
  ...
  g_RootDirectory = (DirectoryEntry *)malloc(
      sectors *
      g_BootSector.BytesPerSector); // readSectors function can only read full
                                    // sectors this way we don't overflow
...
bool readFile(DirectoryEntry *fileEntry, FILE *disk, uint8_t *outputBuffer) {
  ...
  do {
    ...
    uint32_t fatIndex = currentCluster * 3 / 2; // each entry is 12 bits wide
    if (currentCluster % 2 == 0)
      currentCluster = (*(uint16_t *)(g_Fat + fatIndex)) & 0x0FFF; // bit mask
    else
